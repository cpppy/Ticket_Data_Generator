...
def gen_bank_id_list(sample_num):
    bank_id_list = []
    num_dict = range(0, 10)
    for i in range(sample_num):
        choosed_num_list = random.choices(num_dict, k=12)
        bank_id_str = ''.join([str(elem) for elem in choosed_num_list])
        bank_id_list.append(bank_id_str)
    return bank_id_list
...
if __name__ == '__main__':
    ...
    params.tarfile_path = tarfile_path = '/data/data/ticket_train_data_1k_bank_id_20190326_init.tar'

    sample_num = params.sample_num
    date_list = gen_bank_id_list(sample_num)
